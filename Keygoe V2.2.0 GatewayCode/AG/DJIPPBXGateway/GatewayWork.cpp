...
int				g_cfgUserCalledMaxLen;
int				g_cfgUserCalledMinLen;
...
//		pIniFilePath = 	chMdlFlName;
	...
	// User Callout CallNum Min Length
	g_cfgUserCalledMaxLen = GetPrivateProfileInt("USERCALLED", "CallNumMaxLen", 30, pIniFilePath);
	g_cfgUserCalledMinLen = GetPrivateProfileInt("USERCALLED", "CallNumMinLen", 4, pIniFilePath);
}
...
DJ_Void	XMSEventProc(DJ_S32 esrParam)
{
	...
	strcat( TmpStr, "  设备类型: ");
	strcat( TmpStr, GetString_DeviceMain( pAcsEvt->m_DeviceID.m_s16DeviceMain) );
	...
	switch(pAcsEvt->m_s32EventType)
	{
		...
		default:
			...
			{
				ProcVOIPWork(
					&g_AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].pVOIP[pAcsEvt->m_DeviceID.m_s16ChannelID], 
					pAcsEvt);
			}
			if (pAcsEvt->m_DeviceID.m_s16DeviceMain == XMS_DEVMAIN_INTERFACE_CH)
			{
				ProcInterfaceWork(
					&g_AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].pInterf[pAcsEvt->m_DeviceID.m_s16ChannelID], 
					pAcsEvt);
			}
			if (pAcsEvt->m_DeviceID.m_s16DeviceMain == XMS_DEVMAIN_VOICE)
			{
				VOICE_STRUCT *pOneVoice = 
					&g_AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].pVoice[pAcsEvt->m_DeviceID.m_s16ChannelID];
				if (pOneVoice->UsedDevID.m_s8ModuleID > 0)
				{
					if (pOneVoice->UsedDevID.m_s16DeviceMain == XMS_DEVMAIN_VOIP)
					{
						VOIP_STRUCT *pOneVOIP = 
							&g_AllDeviceRes[pOneVoice->UsedDevID.m_s8ModuleID].pVOIP[pOneVoice->UsedDevID.m_s16ChannelID];

						if(pOneVOIP->LinkDevID.m_s8ModuleID > 0)
						{
							ProcInterfaceWork(
								&g_AllDeviceRes[pOneVOIP->LinkDevID.m_s8ModuleID].pInterf[pOneVOIP->LinkDevID.m_s16ChannelID], pAcsEvt);
						}
					}
		...
		else
		{
			...
			for(int i = 0; i < s32Num; ++i)
			{
				...
				case XMS_DEVSUB_ANALOG_USER:	// 内线
					{
						g_AllDeviceRes[s8DspModID].pInterf[i].iState = USER_WAITOPEN;
					}
					break;
				case XMS_DEVSUB_ANALOG_TRUNK:	// 外线
					{
						g_AllDeviceRes[s8DspModID].pInterf[i].iState = ATRK_WAITOPEN;
					}
					break;
...
/**********************************************************************************/
	...
	for(int i = 0; i < g_AllDeviceRes[s8DspModID].lPcmNum; ++i)
	{
		if( CfgIsUsePcm(s8DspModID, i) )
		{
			...
			{
				// 当为模拟线通道时 打开通道资源
				if(g_AllDeviceRes[s8DspModID].pInterf[j].deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_TRUNK ||
					g_AllDeviceRes[s8DspModID].pInterf[j].deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_USER )
				{
					XMS_ctsOpenDevice(g_acsHandle, &g_AllDeviceRes[s8DspModID].pInterf[j].deviceID, NULL);
					// 打开模拟接口用的语音资源
					int iVocIndex = g_AllDeviceRes[s8DspModID].lVOIPNum + iAorUSum;
					g_AllDeviceRes[s8DspModID].pInterf[j].iAIndex = iAorUSum;
					++iAorUSum;
					XMS_ctsOpenDevice(g_acsHandle, &g_AllDeviceRes[s8DspModID].pVoice[iVocIndex].deviceID, NULL);

					memcpy(&g_AllDeviceRes[s8DspModID].pInterf[j].VocDevID, 
						&g_AllDeviceRes[s8DspModID].pVoice[iVocIndex].deviceID, sizeof(DeviceID_t));
					memcpy(&g_AllDeviceRes[s8DspModID].pVoice[iVocIndex].UsedDevID, 
						&g_AllDeviceRes[s8DspModID].pInterf[j].deviceID, sizeof(DeviceID_t));

					strSecString.Format("%d-%d-%d", s8DspModID, i, j);
					iPhoneNum = s8DspModID*1000 + i*100 + j;
					strPhoneNumString.Format("%d", iPhoneNum);
					GetPrivateProfileString("PHONE", strSecString, strPhoneNumString, 
						g_AllDeviceRes[s8DspModID].pInterf[j].SelfCallNum, CALL_NUMBER_MAX_LENGTH, g_strCfgIni);
					WritePrivateProfileString("PHONE", strSecString, 
						g_AllDeviceRes[s8DspModID].pInterf[j].SelfCallNum, g_strCfgIni);
				}
			}
...
/**********************************************************************************/
		...
		*/
		...
		switch(pOneTrunk->deviceID.m_s16DeviceSub)
		{
		case XMS_DEVSUB_ANALOG_USER:
			{
				int iVocIndex = g_AllDeviceRes[pDevice->m_s8ModuleID].lVOIPNum + pOneTrunk->iAIndex;
				g_AllDeviceRes[pDevice->m_s8ModuleID].pVoice[iVocIndex].UsedDevID.m_CallID = pDevice->m_CallID;
				pOneTrunk->iState = USER_FREE;
				XMS_ctsResetDevice(g_acsHandle, pDevice, NULL);
				XMS_ctsGetDevState(g_acsHandle, pDevice, NULL);
			}
			break;
		...
		case XMS_DEVSUB_ANALOG_TRUNK:
			{
				...
				if( g_AllDeviceRes[pDevice->m_s8ModuleID].pVoice[iVocIndex].iState == VOC_FREE )
				{
					// 20080506 USER底层修改后 初始化ATRUNK
					InitATrunk(pOneTrunk);
					/*// 进行Trunk与Voice的绑定初始化 ...
					...
					cmdAnalogTrunk.m_VocDevID = g_AllDeviceRes[pDevice->m_s8ModuleID].pVoice[iVocIndex].deviceID;

					DJ_U16 u16ParamType = ANALOGTRUNK_PARAM_UNIPARAM;
					DJ_U16 u16ParamSize = sizeof(CmdParamData_AnalogTrunk_t);

					RetCode_t iResult = XMS_ctsSetParam(g_acsHandle, &pOneTrunk->deviceID, 
						u16ParamType, u16ParamSize, &cmdAnalogTrunk);*/

					g_AllDeviceRes[pDevice->m_s8ModuleID].pVoice[iVocIndex].iState = VOC_USED;
				}
			}
			break;

		default:	// 注意此时 pOneTrunk->deviceID.m_s16DeviceSub 不能为 XMS_DEVSUB_ANALOG_EMPTY
			{
				pOneTrunk->iState = TRK_FREE;
				XMS_ctsResetDevice(g_acsHandle, pDevice, NULL);
				XMS_ctsGetDevState(g_acsHandle, pDevice, NULL);
...
/**********************************************************************************/
		...
		*/
		...
		if( CfgIsUseVOIP(pDevice->m_s8ModuleID) && 
			pDevice->m_s16ChannelID < g_AllDeviceRes[pDevice->m_s8ModuleID].lVOIPNum )
		{
			g_AllDeviceRes[pDevice->m_s8ModuleID].pVOIP[pDevice->m_s16ChannelID].VocDevID.m_CallID = 
				pDevice->m_CallID;
		}
		...
		if( iAIndex >= 0 )
		{
			for(int i = 1; i < g_AllDeviceRes[pDevice->m_s8ModuleID].lInterfNum; ++i)// i = 0 or 16 已停用
			{
				...
				{
					g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].VocDevID.m_CallID = 
						pDevice->m_CallID;
					if( g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].iState == ATRK_FREE && 
						g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].deviceID.m_s16DeviceSub == 
						XMS_DEVSUB_ANALOG_TRUNK)
					{
						// 20080506 USER底层修改后 初始化ATRUNK
						InitATrunk(&g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i]);
						/*// 进行ATrunk与Voice的绑定初始化 ...
						CmdParamData_AnalogTrunk_t cmdAnalogTrunk;
						memset(&cmdAnalogTrunk, 0, sizeof(cmdAnalogTrunk));
						// 20080411 MODIFY BY WHA FOR WZK
						cmdAnalogTrunk.m_u8CallOutMode = 1;
						...
						//cmdAnalogTrunk.m_u8DialtoneDetect = XMS_ANALOG_TRUNK_DIALTONE_DETECT;
						//cmdAnalogTrunk.m_u8CallOutMethod = XMS_ANALOG_TRUNK_CALLOUT_NORMAL_PROGRESS;
						cmdAnalogTrunk.m_VocDevID = pOneVoice->deviceID;

						DJ_U16 u16ParamType = ANALOGTRUNK_PARAM_UNIPARAM ;
						DJ_U16 u16ParamSize = sizeof(CmdParamData_AnalogTrunk_t);

						RetCode_t iResult = XMS_ctsSetParam(g_acsHandle, 
							&g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].deviceID, 
							u16ParamType, u16ParamSize, &cmdAnalogTrunk);*/

//						if(g_cfgAppIsLog)
//						{
//							sprintf(chTemp256, "(Interface DSPID%d ChannelID%d) Link (Voice DSPID%d ChannelID%d) Return %d", 
//								g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].deviceID.m_s8ModuleID, 
//								g_AllDeviceRes[pDevice->m_s8ModuleID].pInterf[i].deviceID.m_s16ChannelID, 
//								pDevice->m_s8ModuleID, pDevice->m_s16ChannelID, 
//								iResult);
//
//							g_pLogInfo->WriteLogInfo(chTemp256);
//						}
		...
		int iResult = XMS_ctsSetParam(g_acsHandle, pDevice, u16ParamType, u16ParamSize, (void*)&cmdVoc);
/**********************************************************************************/
	}
...
void AddDevLineState(Acs_Evt_t *pAcsEvt)
{
	...
	if ( pAcsEvt->m_DeviceID.m_s16DeviceMain == XMS_DEVMAIN_INTERFACE_CH )
	{
		...
		// 20080506 底层修改以后添加状态改变事件在具体处理过程中的使用
		if( pOneInterf->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_USER )
		{
			ProcUserWork(pOneInterf, pAcsEvt);
		}
		else if( pOneInterf->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_TRUNK )
		{
			ProcATrunkWork(pOneInterf, pAcsEvt);
		}
	}
