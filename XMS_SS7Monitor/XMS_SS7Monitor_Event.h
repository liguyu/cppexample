...
#define		M_OneSS7Link(DevID)		AllDeviceRes[(DevID).m_s8ModuleID].pSS7Link[(DevID).m_s16ChannelID]
...
typedef	struct
{
	...
	long			lSS7LinkNum;			// the XMS_DEVMAIN_DIGITAL_PORT number in this DPS
	long			lSS7LinkOpened;			// the PCM number opened by OpenDevice()
	SS7LINK_STRUCT		*pSS7Link;		// the structer of PCM, alloc as need
