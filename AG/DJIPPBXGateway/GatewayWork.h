...
typedef struct tagINTERFACE_STRUCT		// 1 CH
{
	...
	DeviceID_t  VocDevID;		// 模拟采用
	int			iIsMakeCallOut;	// 模拟采用 MakeCallOut调用后的标志
	DWORD		dwTime;
	int			iAIndex;		// 每个DSP下模拟接口的序号
	char		SelfCallNum[CALL_NUMBER_MAX_LENGTH];	// 本通道号码
...
extern int				g_cfgUserCalledMaxLen;					// USER CALLOUT CALLNUM MAX LENGTH
extern int				g_cfgUserCalledMinLen;					// USER CALLOUT CALLNUM MIN LENGTH
...
void		ProcATrunkWork(INTERFACE_STRUCT* pOneTrunk, Acs_Evt_t* pAcsEvt);		// ATrunk Work Proc
void		ProcUserWork(INTERFACE_STRUCT* pOneTrunk, Acs_Evt_t* pAcsEvt);			// User Work Proc
...
int			SearchOneFreeIntfDTrunk(VOIP_STRUCT *pOneVOIP, DeviceID_t &pFreeIntfDeviceID);// 找到当前一个空闲的Intf资源
int			SearchOneFreeIntfUserOrAtrunk(VOIP_STRUCT *pOneVOIP, DeviceID_t &pFreeIntfDeviceID);// 找到当前一个空闲的Intf资源
...
int			PlayTone(DeviceID_t	*pVocDevID, int iPlayType);			// 用户线放音函数

// 20080506 USER底层修改后 初始化的更改
int			InitATrunk(INTERFACE_STRUCT* pOneATrunk);				// 初始化设置ATRUNK
int			ATrunkCallInProc(INTERFACE_STRUCT* pOneTrunk);			// ATrunk CallIn时通过VOIP呼出处理函数

typedef struct tagINISTRUCT
{
	/* Keygoe服务的配置信息 */
	char		XMSIP[16];			// 默认值 本配置设置的Keygoe服务IP		例如"192.168.10.33"
	int			XMSPORT;			// 默认值 本配置设置的Keygoe服务PORT	例如9000
	char		XMSUSER[20];		// 默认值 本配置设置的Keygoe服务USER	例如""
	char		XMSPASSWORD[20];	// 默认值 本配置设置的Keygoe服务PSWD	例如""
	int			XMSAPPID;			// 默认值 111
	int			XMSRCVQUESIZE;		// 默认值 64
	int			XMSSNDQUESIZE;		// 默认值 64
	int			XMSDEBUGON;			// 默认值 0								可选值 0 or 1
	char		VOIPDSPID[40];		// 默认值 "1"
	char		DIGIDSPID[80];		// 默认值 "1-0"
	int			ISWRITELOGFILE;		// 默认值 0								可选值 0 or 1

	/* 默认SIP SERVER的配置信息 */
	char		SIPSVRIP[16];		// 默认值 "0.0.0.0"
	int			SIPSVRPORT;			// 默认值 5060
	int			SIPSVRPROTOCOL;		// 默认值 2								可选值 "NOP" "H.323" "SIP"

	/* VOIP通过网关转换为中继方向的路由以及号码转换配置信息 */
	int			VTROUTINGSUM;		// 默认值 0
	struct tagVTROUTING
	{
		char CALLNUMSTRING[20];		// 默认值 ""
		char E1STRING[80];			// 默认值 ""
	} VTROUTING[16];
	int			VTCHANGECALLERSUM;	// 默认值 0
	char		VTCHANGECALLERSTRING[16][20];	// 默认值 ""

	/* 中继通过网关转换为VOIP方向的路由以及号码转换配置信息 */
	int			TVROUTINGSUM;		// 默认值 0
	struct tagTVROUTING
	{
		char CALLNUMSTRING[20];		// 默认值 ""
		char IP[16];				// 默认值 ""
		int  PORT;					// 默认值 0
		int  PROTOCOL;				// 默认值 0								可选值 "NOP" "H.323" "SIP"
	} TVROUTING[16];
	int			TVCHANGECALLERSUM;	// 默认值 0
	char		TVCHANGECALLERSTRING[16][20];	// 默认值 ""

	int			USERCALLOUTNUMLEN;	// 默认值 4
	int			SEARCHVOIPRULE;		// 默认值 0								可选值 0 or 1

	char		ATORUSERNUM[30][10];// 默认值 ""

	char		EX[604];			// 预留字段 结构总长为4K

	tagINISTRUCT(){memset(this, 0, sizeof(tagINISTRUCT));};
}INISTRUCT, *LPINISTRUCT;
