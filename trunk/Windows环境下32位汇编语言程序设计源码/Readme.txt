研砉程序园nihg为您上传这个罗大哥的配套光盘,欢迎光顾我们
http://www.aogosoft.com 和我的小站 http://nihg.net 

×××××××××××××××××××××××××××
×  《Windows 环境下32位汇编程序设计》附书代码说明  ×
×               http://asm.yeah.net                ×
×××××××××××××××××××××××××××
...
Chapter04\MsgWindow01              ;消息机制试验 1
Chapter04\MsgWindow02              ;消息机制试验 2
Chapter04\MsgWindow03              ;消息机制试验 3
Chapter04\MsgWindow04              ;消息机制试验 4
...
Chapter08\BrowseFolder             ;浏览目录对话框
...
Chapter10\WordCount                ;文件读写例子
Chapter10\WordCount\FileMap        ;使用内存映射文件进行文件读写的例子
Chapter10\MMFShare                 ;使用内存映射文件进行进程间数据共享

Chapter11\Dll                      ;最简单的动态链接库例子 - 使用 DLL
...
Chapter12\ThreadSyn                ;解决了同步问题的多线程程序
...
Chapter16\Chat-UDP                 ;用 UDP 协议实现的聊天室例子
Chapter16\Ping                     ;用 ICMP 协议实现的 Ping 例子
