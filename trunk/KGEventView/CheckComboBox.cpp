...
void CCheckComboBox::UnSelectAll()
{
    INT nCount = GetCount();
...
void CCheckComboBox::OnSelectChange()
{
    BOOL  select;
	if(g_mslect==0) 
	{
      select=m_pComboBox->GetCheck(0);
	  if(select) m_pComboBox->SelectAll();
	  else 
	  {
		  m_pComboBox->UnSelectAll();
	  }
 	}
}
