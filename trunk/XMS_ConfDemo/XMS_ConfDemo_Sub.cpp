...
void DrawIP_CallInfo( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	...
	sprintf ( StateStr,"%s:%d", pOneVOIP->addrCaller.tszAddr,pOneVOIP->addrCaller.port );
	pdlg->m_ListIP.SetItemText ( iDispRow, 7, StateStr ); 
	
	strcpy ( StateStr, pOneVOIP->addrCaller.tszUserID);
	pdlg->m_ListIP.SetItemText ( iDispRow, 8, StateStr ); 
}
...
void DrawIP_DTMF( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	...
	pdlg->m_ListIP.SetItemText ( iDispRow, 6, StateStr ); 
...
void DrawIP_VocInfo( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	DeviceID_t	*pDev;
	...
	pdlg->m_ListIP.SetItemText ( iDispRow, 4, StateStr ); 
	
}
void DrawIP_ConfInfo( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	DeviceID_t	*pDev;
	...
	if ( pOneVOIP->ConfDevID.m_s16DeviceMain != 0 )
	{
		pDev = &pOneVOIP->ConfDevID;
		sprintf ( StateStr, "%d,%d", pDev->m_s8ModuleID, pDev->m_s16ChannelID ); 
	}
	else
		strcpy(StateStr, "");

	pdlg->m_ListIP.SetItemText ( iDispRow, 5, StateStr ); 
...
void	InitListIP(void)
{
	...
	dwExStyle = LVS_EX_FULLROWSELECT | /*LVS_EX_GRIDLINES | LVS_EX_SUBITEMIMAGES |*/
		LVS_EX_HEADERDRAGDROP;// | LVS_EX_TRACKSELECT;
	pdlg->m_ListIP.SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0, LPARAM(dwExStyle));	
	...
	lvc.pszText = "#" ;
	lvc.cx = 30;
	pdlg->m_ListIP.InsertColumn ( 0, &lvc ); 
	
	lvc.iSubItem = 1;
	lvc.pszText = "Mod,Ch" ;
	lvc.cx = 50;
	pdlg->m_ListIP.InsertColumn ( 1, &lvc ); 
	
	lvc.iSubItem = 2;
	lvc.pszText = "State" ;
	lvc.cx = 70;
	pdlg->m_ListIP.InsertColumn ( 2, &lvc ); 

	lvc.iSubItem = 3;
	lvc.pszText = "Protocol" ;
	lvc.cx = 60;
	pdlg->m_ListIP.InsertColumn ( 3 ,&lvc ); 	
	
	lvc.iSubItem = 4;
	lvc.pszText = "VocInfo" ;
	lvc.cx = 50;
	pdlg->m_ListIP.InsertColumn ( 4, &lvc );

	lvc.iSubItem = 5;
	lvc.pszText = "ConfInfo" ;
	lvc.cx = 60;
	pdlg->m_ListIP.InsertColumn ( 5, &lvc );

	lvc.iSubItem = 6;
	lvc.pszText = "DTMF" ;
	lvc.cx = 85;
	pdlg->m_ListIP.InsertColumn ( 6, &lvc ); 
	
	lvc.iSubItem = 7;
	lvc.pszText = "Caller Address" ;
	lvc.cx = 80;
	pdlg->m_ListIP.InsertColumn ( 7, &lvc ); 
	
	lvc.iSubItem = 8;
	lvc.pszText = "Caller UserID" ;
	lvc.cx = 80;
	pdlg->m_ListIP.InsertColumn ( 8, &lvc ); 
	
	lvc.iSubItem = 9;
	lvc.pszText = "Caller TelNo" ;
	lvc.cx = 80;	
	pdlg->m_ListIP.InsertColumn ( 9, &lvc ); 

	lvc.iSubItem = 10;
	lvc.pszText = "Callee Address" ;
	lvc.cx = 80;
	pdlg->m_ListIP.InsertColumn ( 10, &lvc ); 
	
	lvc.iSubItem = 11;
	lvc.pszText = "Callee UserID" ;
	lvc.cx = 80;
	pdlg->m_ListIP.InsertColumn ( 11, &lvc ); 
	
	lvc.iSubItem = 12;
	lvc.pszText = "Callee TelNo" ;
	lvc.cx = 80;	
	pdlg->m_ListIP.InsertColumn ( 12, &lvc ); 
...
void DrawIP_State( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	...
	case VOIP_CONF_PLAYBACK:
		strcpy(StateStr,"PlayBack");
		break;
		
	case VOIP_CONF_ERROR:
		strcpy(StateStr,"ConfErr");
		break;
	...
	case VOIP_WAIT_LINKOK:
		strcpy(StateStr,"Wait LinkOK");
		break;
		
	case VOIP_WAIT_REMOVE:
		strcpy(StateStr,"Remove DSP");
		break;
	...
	pdlg->m_ListIP.SetItemText ( iDispRow, 2, StateStr ); 
}

void DrawIP_Protocol( VOIP_STRUCT *pOneVOIP )
{
	char StateStr[100];
	int	 iDispRow;
	iDispRow = pOneVOIP->iSeqID; 
	switch( pOneVOIP->iProtocol ) 
	{
	case XMS_VOIP_PROTOCOL_H323:
		strcpy(StateStr,"H.323"); 
		break;
	...
	pdlg->m_ListIP.SetItemText ( iDispRow, 3, StateStr ); 
}
...
void	InitListCount(void)
{
	...
	lvc.cx = 70;
	pdlg->m_ListCount.InsertColumn ( 10, &lvc ); 
	
	lvc.iSubItem = 11;
	lvc.pszText = "VOIPFree" ;
	lvc.cx = 60;
	pdlg->m_ListCount.InsertColumn ( 11, &lvc ); 
}
...
void	DrawCount_VOIP ( DJ_S8	s8ModID )
{
	...
	pdlg->m_ListCount.SetItemText ( iDispRow, 10, TmpStr );
...
void	OpenVOIPDevice ( VOIP_STRUCT *pOneVOIP )
{
	...
	if ( pOneVOIP->State == VOIP_WAITOPEN ) 		// not Open yet
	{
...
void	OpenDeviceOK ( DeviceID_t *pDevice )
{
	...
	{
		M_OneVOIP(*pDevice).deviceID.m_CallID = pDevice->m_CallID;		// this is very important
		
		XMS_ctsResetDevice ( g_acsHandle, pDevice, NULL );
		XMS_ctsGetDevState ( g_acsHandle, pDevice, NULL );
		
		InitVOIPChannel ( &M_OneVOIP(*pDevice) );
...
void	CloseDeviceOK ( DeviceID_t *pDevice )
{
	...
	if ( pDevice->m_s16DeviceMain == XMS_DEVMAIN_VOIP )
	{
		M_OneVOIP(*pDevice).State = VOIP_WAITOPEN;
...
void My_InitDtmfBuf_VOIP ( VOIP_STRUCT *pOneVOIP )
{
...
void My_AddDtmfBuf_VOIP ( VOIP_STRUCT *pOneVOIP, char TmpDtmf )
{
...
char My_GetDtmfCode_VOIP ( Acs_Evt_t *pAcsEvt )
{
	Acs_IO_Data				*pIOData = NULL;
	char		*p;
	...
	if ( pAcsEvt->m_s32EventType == XMS_EVT_RECVIODATA )	/*IO Data事件*/
	{
		...
		{
			if (	pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_REGREQUEST ||
					pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_REGSTATE ||
					pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_H245UII ||
					pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_SIPINFO ||
					pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_RTPDTMF ||
					pIOData->m_u16IoType == XMS_IO_TYPE_VOIP_RTPTONE )
			{
				p = (char *)FetchIOData(pAcsEvt);
				return *p;
			}
...
void	Change_VOIP_State ( VOIP_STRUCT *pOneVOIP, VOIP_STATE NewState )
{
	...
	// Check if ready to remove DSP hardware
	if ( (AllDeviceRes[pOneVOIP->deviceID.m_s8ModuleID].RemoveState == DSP_REMOVE_STATE_START)
		&& (NewState == TRK_FREE) )
	{
		pOneVOIP->State = VOIP_WAIT_REMOVE;
		
		DrawIP_State ( pOneVOIP );
		
		CheckRemoveReady ( pOneVOIP->deviceID.m_s8ModuleID );
	}
}

void	InitVOIPChannel ( VOIP_STRUCT *pOneVOIP )
{
	Change_VOIP_State ( pOneVOIP, VOIP_FREE);
	pOneVOIP->DtmfCount = 0;
	pOneVOIP->DtmfBuf[0] = 0; 
	DrawIP_DTMF ( pOneVOIP );

	memset ( &pOneVOIP->VocDevID, 0, sizeof(DeviceID_t) );		// 0: didn't alloc Device
	DrawIP_VocInfo ( pOneVOIP );
	
	memset ( &pOneVOIP->ConfDevID, 0, sizeof(DeviceID_t) );		// 0:didn't alloc  Device
	DrawIP_ConfInfo ( pOneVOIP );
...
void	Do_JoinConf_VOIP ( VOIP_STRUCT *pOneVOIP, DeviceID_t *pConfDevID )
{
	char					FileName[256];
	...
	//free the original Voc Device bind with voip
	My_DualUnlink ( &pOneVOIP->deviceID, &pOneVOIP->VocDevID );
	FreeOneVoice ( &pOneVOIP->VocDevID );		
	memset ( &M_OneVoice(pOneVOIP->VocDevID).UsedDevID,	0, sizeof(DeviceID_t) );		// 0: didn't alloc  Device
	memset ( &pOneVOIP->VocDevID, 0, sizeof(DeviceID_t) );		// 0: didn't alloc  Device
	DrawIP_VocInfo ( pOneVOIP );

	// remember now voip & Conf Device
	pOneVOIP->ConfDevID = *pConfDevID;
	...
	case 1:		// add				
		confParam.m_u8InputOpt = XMS_CONF_INPUT_OPT_NORMAL;
		confParam.m_u8OutputOpt = XMS_CONF_OUTPUT_OPT_NORMAL;
		ret = XMS_ctsJoinToConf(g_acsHandle,pConfDevID,&pOneVOIP->deviceID,&confParam, NULL);
		break;
...
void TrunkWork_VOIP ( VOIP_STRUCT *pOneVOIP, Acs_Evt_t *pAcsEvt )
{
	...
	case VOIP_FREE:
		...
		if ( pAcsEvt->m_s32EventType == XMS_EVT_CALLIN )	/*Call In*/
		{
			...
			pOneVOIP->iProtocol = paramVoIPCall->m_s32Protocol;
			sprintf( pOneVOIP->addrCaller.tszTelNo, pCallEvt->m_s8CallingNum);
			sprintf( pOneVOIP->addrCaller.tszAddr, paramVoIPCall->m_s8CallerAddress);
			pOneVOIP->addrCaller.port = (USHORT)paramVoIPCall->m_s32CallerPort;
			sprintf( pOneVOIP->addrCaller.tszUserID, paramVoIPCall->m_s8CallerUserID);

			sprintf( pOneVOIP->addrCallee.tszTelNo, pCallEvt->m_s8CalledNum);
			sprintf( pOneVOIP->addrCallee.tszAddr, paramVoIPCall->m_s8CalleeAddress);
			pOneVOIP->addrCallee.port = (USHORT)paramVoIPCall->m_s32CalleePort;
			sprintf( pOneVOIP->addrCallee.tszUserID, paramVoIPCall->m_s8CalleeUserID);

			
			XMS_ctsAlertCall ( g_acsHandle, &pOneVOIP->deviceID, NULL );
			XMS_ctsAnswerCallIn ( g_acsHandle, &pOneVOIP->deviceID, NULL );
			
			DrawIP_CallInfo( pOneVOIP );
			DrawIP_Protocol( pOneVOIP );
			Change_VOIP_State ( pOneVOIP, VOIP_WAIT_ANSWERCALL );
		}
		break;
	...
	case VOIP_WAIT_ANSWERCALL:
		if ( pAcsEvt->m_s32EventType == XMS_EVT_ANSWERCALL )	/*Answer Call In OK*/
		{
			...
			if ( SearchOneFreeVoice ( &FreeVocDeviceID, SEARCH_RULE_MAX_FREE_FIST, 0 ) >= 0 )
			{
				...
				DrawIP_VocInfo(pOneVOIP);
				My_DualLink ( &pOneVOIP->deviceID, &FreeVocDeviceID );
				...
				Change_VOIP_State ( pOneVOIP, VOIP_WAIT_LINKOK );
			}
		}
		break;
	...
	case VOIP_WAIT_LINKOK:
		if ( pAcsEvt->m_s32EventType == XMS_EVT_LINKDEVICE )	/*LinkDevice OK*/
		{
			...
			Change_VOIP_State ( pOneVOIP, VOIP_SELECT );
		}
		break;
	case VOIP_SELECT:
		...
		if ( TmpDtmf == '1' || TmpDtmf == '2' )	 // "1: Add Conf" "2: Listen Conf" "3: Karaok" "4 Speak Only"
		{
			...
			pOneVOIP->iConfSelect = TmpDtmf - '1' + 1;
...
// 		}
	...
	case VOIP_CONF_INPUTGROUP:
		...
		if ( pOneVOIP->DtmfCount >= 3 )
		{
			sscanf ( pOneVOIP->DtmfBuf, "%d", &iConfNo ); 
			if ( (iConfNo < 0) || (iConfNo >= g_iTotalConf) )
			{
				Change_VOIP_State ( pOneVOIP, VOIP_CONF_ERROR );
				break;
