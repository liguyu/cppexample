...
#include "D:\\cppexample\\XMS_TestDial\\CAS_Common_Cfg.H"		
...
void	OpenAllDevice_Dsp ( DJ_S8 s8DspModID )
{
	...
	// pVoice
	for ( j = 0; j < AllDeviceRes[s8DspModID].lVocNum; j++ )
	{
		OpenVoiceDevice ( &AllDeviceRes[s8DspModID].pVoice[j] );
	}

	// pPcm
	for ( k = 0; k < AllDeviceRes[s8DspModID].lPcmNum; k++ )
	{
		OpenPcmDevice ( &AllDeviceRes[s8DspModID].pPcm[k] );
	}
}
...
//		pOneTrunk->DtmfCount ++;
		// TRACE("pOneTrunk->DtmfCount = %s,TmpDtmf = %s",pOneTrunk->DtmfCount,TmpDtmf);
	}
}
...
void TrunkWork ( TRUNK_STRUCT *pOneTrunk, Acs_Evt_t *pAcsEvt )
{
	...
	if ( pAcsEvt->m_s32EventType == XMS_EVT_CLEARCALL )	/*拆线事件*/
	{
		if (pOneTrunk->State == TRK_WAITHANGUP)
		{
			if (cfg_iVoiceCheck == 1)
			{
			//shuxc	StopRecordFile(&pOneTrunk->VocDevID);
		...
		//		DeleteFile(FileName); //删除录音文件
				...
				{
				//shuxc	sprintf (TmpFileName, "%s.%d.pcm", FileName,pOneTrunk->iTotalCnt);
				...
				{
					pOneTrunk->iSucCnt++;
		//shuxc			DeleteFile(FileName);//删除录音文件
				}			
	...
	case TRK_PLAY:
		...
		//	GetFilePathName(pOneTrunk, TmpFileName);
			//sprintf(FileName, "%s/DemoRec.%0003d", TmpFileName, pOneTrunk->iSeqID);		
			//shuxc RecordFile ( &pOneTrunk->VocDevID, FileName, 8000L*15, false);
...
//shuxc	if (CheckRecordEnd(pOneTrunk, pAcsEvt))
//		{
		//	XMS_ctsClearCall ( g_acsHandle, &pOneTrunk->deviceID, 0, NULL );
		
//		}
		if ( pOneTrunk->DtmfCount >= 10 ){//输入四位分机号
			Change_State (pOneTrunk, TRK_WAITHANGUP);
		}
